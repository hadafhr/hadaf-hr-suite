...
import { Building2, Users, DollarSign, Settings, Bell, Search, Plus, TrendingUp, Activity, AlertCircle, CheckCircle, Clock, BarChart3, FileText, Shield, User, LogOut, ChevronDown, Menu, Database, Globe, Zap, Crown, RefreshCw, Monitor, Server } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
  ...
  const sidebarItems = [{
    ...
    label: isArabic ? 'تطوير النظام' : 'System Development',
    icon: Zap,
  ...
  const systemAlerts = [{
    id: 1,
    type: 'warning',
    title: isArabic ? 'استخدام عالي للخادم' : 'High Server Usage',
    description: '85% CPU usage detected',
    urgent: true
  }, {
    id: 2,
    type: 'info',
    title: isArabic ? 'عميل جديد' : 'New Client Registration',
    description: 'Pending approval',
    urgent: false
  }, {
    id: 3,
    type: 'success',
    title: isArabic ? 'تم التحديث' : 'System Updated',
    description: 'Version 2.1.5 deployed',
    urgent: false
  }];
      ...
      <header className="fixed top-0 right-0 left-0 z-50 h-20 bg-gradient-to-r from-black/95 via-card/95 to-black/95 backdrop-blur-xl border-b border-border shadow-2xl shadow-accent/20">
        ...
        <div className="relative z-10 h-full flex items-center justify-between px-6">
          ...
          <div className="flex items-center space-x-4 space-x-reverse">
            <Button variant="ghost" size="sm" onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden text-white hover:bg-accent/20 p-2 rounded-full">
          ...
          <div className="hidden lg:flex items-center space-x-6 space-x-reverse">
            ...
            <div className="relative hidden xl:block">
              ...
              <Input placeholder={isArabic ? 'البحث السريع...' : 'Quick search...'} className="pl-10 w-64 h-9 border-border focus:border-accent/50 bg-black/30 text-white placeholder:text-muted-foreground rounded-full backdrop-blur-sm" />
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 space-x-reverse">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-80 bg-black/95 backdrop-blur-xl border border-border text-white">
                ...
                {systemAlerts.map(alert => <DropdownMenuItem key={alert.id} className="p-4 hover:bg-accent/20">
          ...
          <nav className="mt-8 px-4 h-full overflow-y-auto">
            <ul className="space-y-2">
              {sidebarItems.map((item, index) => <li key={index}>
                  <button onClick={() => navigate(item.route)} className={`w-full flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors ${item.active ? 'bg-gradient-to-r from-accent to-accent text-black shadow-lg' : 'text-muted-foreground hover:text-white hover:bg-accent/20'}`}>
          ...
          <main className="p-6 relative z-10">
            <div className="max-w-7xl mx-auto space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {systemStats.map((stat, index) => <Card key={index} className="p-6 backdrop-blur-xl bg-black/40 border border-border hover:shadow-2xl hover:shadow-accent/20 transition-all duration-300 hover:scale-105 animate-fade-in rounded-2xl">
