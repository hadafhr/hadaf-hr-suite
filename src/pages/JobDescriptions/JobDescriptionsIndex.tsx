    ...
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden" dir="rtl">
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 relative">
          <div className="flex items-center justify-between h-24">
            ...
            <div className="flex flex-col items-end space-y-4">
              {/* Status Panel */}
              <div className="bg-gradient-to-r from-background/40 via-card/60 to-background/40 backdrop-blur-xl rounded-2xl border border-border shadow-xl shadow-accent/10 p-4 min-w-[200px]">
                {/* Status Indicator */}
              ...
              <div className="bg-gradient-to-r from-background/20 to-card/30 backdrop-blur-lg rounded-xl border border-border px-3 py-2 shadow-lg">
                <div className="flex items-center space-x-3 text-xs">
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-muted-foreground">{filteredJobs.length} وظيفة</span>
                  </div>
      ...
      <main className="relative z-10 w-full mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12 relative">
          ...
          <h2 className="text-5xl font-bold mb-8 text-foreground bg-gradient-to-r from-foreground via-foreground/90 to-foreground bg-clip-text text-transparent leading-tight">
            دليل الأوصاف الوظيفية
          </h2>
        ...
        <div className="mb-12 space-y-6">
          <div className="flex flex-col lg:flex-row gap-6 items-center justify-center">
            ...
            <div className="relative w-full max-w-2xl group">
              <div className="absolute inset-0 bg-gradient-to-r from-accent/20 to-accent/10 rounded-2xl blur opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              <div className="relative flex items-center">
                ...
                <Input
                  ...
                  className="w-full pr-12 pl-6 h-14 bg-gradient-to-r from-card/80 to-background/60 backdrop-blur-xl border border-border rounded-2xl text-foreground placeholder-muted-foreground focus:border-accent/70 focus:ring-2 focus:ring-accent/30 transition-all duration-300 shadow-xl hover:shadow-accent/20"
                />
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <Button 
                ...
                className="bg-gradient-to-r from-card/50 to-background/30 border border-border text-foreground hover:bg-gradient-to-r hover:from-accent/20 hover:to-accent/10 hover:border-accent/70 transition-all duration-300 shadow-lg hover:shadow-accent/25 px-6 h-12"
              >
            ...
            {['مدير الموارد البشرية', 'محاسب مالي', 'مطور برمجيات', 'مندوب مبيعات'].map((job) => (
              <Badge 
                ...
                className={`cursor-pointer px-6 py-3 text-sm font-semibold transition-all duration-300 ${
                  searchTerm === job 
                    ? 'bg-gradient-to-r from-accent to-accent/80 text-accent-foreground border-accent shadow-lg hover:shadow-accent/25' 
                    : 'bg-gradient-to-r from-card/50 to-background/30 border border-border text-foreground hover:bg-gradient-to-r hover:from-accent/20 hover:to-accent/10 hover:border-accent/70'
                }`}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/4">
            <div className="bg-gradient-to-br from-card/60 to-background/40 backdrop-blur-xl rounded-2xl border border-border shadow-xl shadow-accent/10 p-6">
              ...
              <div className="space-y-6">
                <div>
                  <label className="text-sm font-medium mb-2 block text-muted-foreground">الإدارة</label>
                  <Select value={selectedDepartment} onValueChange={setSelectedDepartment}>
                    <SelectTrigger className="bg-gradient-to-r from-card/50 to-background/30 border border-border text-foreground hover:border-accent/70 transition-all duration-300">
                      <SelectValue placeholder="اختر الإدارة" />
                    </SelectTrigger>
                    <SelectContent className="bg-card border-border">
                      {departments.map((dept) => (
                        <SelectItem key={dept} value={dept} className="text-foreground hover:bg-accent/20">{dept}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block text-muted-foreground">المستوى المهني</label>
                  <Select value={selectedLevel} onValueChange={setSelectedLevel}>
                    ...
                    <SelectContent className="bg-card border-border">
                      {levels.map((level) => (
                        <SelectItem key={level} value={level} className="text-foreground hover:bg-accent/20">{level}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block text-muted-foreground">نوع العمل</label>
                  <Select value={selectedWorkType} onValueChange={setSelectedWorkType}>
                    ...
                    <SelectContent className="bg-card border-border">
                      {workTypes.map((type) => (
                        <SelectItem key={type} value={type} className="text-foreground hover:bg-accent/20">{type}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  ...
                  <Select value={selectedCity} onValueChange={setSelectedCity}>
                    ...
                    <SelectContent className="bg-card border-border">
                      {cities.map((city) => (
                        <SelectItem key={city} value={city} className="text-foreground hover:bg-accent/20">{city}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
