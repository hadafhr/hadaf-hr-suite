...
export const ExpenseRequestsManager: React.FC<ExpenseRequestsManagerProps> = ({ isRTL }) => {
  ...
  const getPriorityConfig = (priority: string) => {
    const priorityConfigs = {
      'low': { color: 'bg-muted/40 text-muted-foreground', text: isRTL ? 'منخفضة' : 'Low' },
      'medium': { color: 'bg-warning/20 text-warning-foreground', text: isRTL ? 'متوسطة' : 'Medium' },
      'high': { color: 'bg-destructive/20 text-destructive-foreground', text: isRTL ? 'عالية' : 'High' }
    };
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 flex-1">
          <div className="relative flex-1 min-w-[300px]">
            ...
            <Input
              ...
              className="bg-card border-border text-foreground pl-10 h-12"
            />
          </div>
          
          <Select value={filterStatus} onValueChange={setFilterStatus}>
            <SelectTrigger className="bg-card border-border text-foreground w-48 h-12">
      ...
      <div className="space-y-4">
        {filteredRequests.map((request) => {
          ...
          return (
            <Card key={request.id} className="bg-card border-border hover:border-accent transition-all duration-300">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-8">
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <h3 className="font-semibold text-foreground text-lg">{request.id}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <p className="text-muted-foreground text-sm">{isRTL ? 'الفئة' : 'Category'}</p>
                        <p className="text-foreground font-medium">{request.category}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground text-sm">{isRTL ? 'المبلغ' : 'Amount'}</p>
                        <p className="text-foreground font-bold text-lg">₩{request.amount.toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground text-sm">{isRTL ? 'تاريخ الطلب' : 'Request Date'}</p>
                        <p className="text-foreground">{request.requestDate}</p>
                      </div>
                    </div>

                    <div>
                      <p className="text-muted-foreground text-sm mb-1">{isRTL ? 'الغرض' : 'Purpose'}</p>
                  ...
                  <div className="flex lg:flex-col gap-2 lg:min-w-[120px]">
                    ...
                    {request.status === 'submitted' && (
                      <>
                        <Button size="sm" className="bg-success text-success-foreground hover:bg-success/90 flex-1 lg:w-full">
