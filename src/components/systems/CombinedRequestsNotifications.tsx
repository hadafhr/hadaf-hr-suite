...
import { ArrowLeft, Megaphone, Bell, MessageSquare, Send, Settings, Eye, Save, Download, Share, Mail, CheckCircle, Clock, AlertTriangle, AlertCircle, Plus, Search, FileText, DollarSign, Target, GraduationCap, Shield, Users, Calendar, Info, XCircle, Trash2, User, CreditCard, Building2, FileCheck, Briefcase, PenTool, Filter, Building, UserPlus, CalendarClock } from 'lucide-react';
...
export const CombinedRequestsNotifications: React.FC<CombinedRequestsNotificationsProps> = ({
  ...
  const filteredRequests = requests.filter(request => {
    const matchesSearch = request.title.toLowerCase().includes(searchTerm.toLowerCase()) || request.description.toLowerCase().includes(searchTerm.toLowerCase()) || request.employee_name.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return <div className="min-h-screen p-6 bg-background text-foreground" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold mb-2 text-foreground">قسم الطلبات والإشعارات</h1>
              ...
              <CardContent className="p-6">
                ...
                <div className="flex flex-wrap items-center gap-4 mb-6">
                  <div className="flex-1 min-w-[200px]">
                    <Input placeholder="البحث في الطلبات..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full" />
                  </div>
                    ...
                    <DialogContent className="max-w-lg" dir="rtl">
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <Select value={requestForm.type} onValueChange={value => setRequestForm({
                          ...requestForm,
                          type: value
                        })}>
                            <SelectTrigger>
                            ...
                            <SelectContent>
                              {requestTypes.map(type => <SelectItem key={type.id} value={type.id}>
                        ...
                        <div className="space-y-2">
                          ...
                          <Input value={requestForm.title} onChange={e => setRequestForm({
                          ...requestForm,
                        ...
                        <div className="space-y-2">
                          ...
                          <Textarea value={requestForm.description} onChange={e => setRequestForm({
                          ...requestForm,
                          description: e.target.value
                        })} placeholder="اشرح تفاصيل طلبك..." rows={3} />
                        </div>

                        {requestForm.type === 'financial_advance' && <div className="space-y-2">
                            <label className="text-sm font-medium">المبلغ المطلوب (ريال)</label>
                            <Input type="number" value={requestForm.amount} onChange={e => setRequestForm({
                          ...requestForm,
                        ...
                        <div className="space-y-2">
                          ...
                          <Select value={requestForm.priority} onValueChange={(value: any) => setRequestForm({
                          ...requestForm,
                          priority: value
                        })}>
                            <SelectTrigger>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-8">
                  {requestTypes.map(type => <div key={type.id} className="text-center group cursor-pointer" onClick={() => setRequestForm({
                  ...requestForm,
                  type: type.id
                })}>
                      <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 shadow-lg group-hover:scale-110 transition-transform bg-primary text-primary-foreground">
                        {type.icon}
                      </div>
                ...
                <ScrollArea className="h-[500px]">
                  <div className="space-y-4">
                    ...
                    return <Card key={request.id} className="bg-card border-border hover:bg-accent/50 transition-all">
                            <CardContent className="p-4">
                              <div className="flex items-start gap-4">
                                <div className="w-12 h-12 rounded-full flex items-center justify-center bg-primary text-primary-foreground flex-shrink-0">
                                  {typeConfig.icon}
                                </div>
                                <div className="flex-1 min-w-0">
                                  ...
                                  {request.amount && <div className="mt-2 text-sm font-medium text-primary">
            ...
            <TabsContent value="notifications">
              <CardContent className="p-6 bg-card border-border rounded-3xl">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      ...
                      {unreadCount > 0 && <Badge className="absolute -top-2 -right-2 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs bg-destructive text-destructive-foreground">
                          {unreadCount}
                        </Badge>}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant={notificationFilter === 'unread' ? 'default' : 'outline'} size="sm" onClick={() => setNotificationFilter('unread')}>
                      غير مقروء ({unreadCount})
                    </Button>
                    <Button variant={notificationFilter === 'urgent' ? 'default' : 'outline'} size="sm" onClick={() => setNotificationFilter('urgent')}>
                      عاجل ({urgentCount})
                    </Button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-8 p-4 bg-card border-border rounded-3xl">
                  ...
                  label: 'الإدارات',
                  section: 'departments'
                }].map((item, index) => <div key={index} className="text-center group cursor-pointer" onClick={() => onNavigateToSection?.(item.section)}>
                      <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2 shadow-lg group-hover:scale-110 transition-transform bg-primary text-primary-foreground">
                ...
                <ScrollArea className="h-[500px]">
                  <div className="space-y-3">
                    {filteredNotifications.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                        ...
                        <p>لا توجد إشعارات</p>
                      </div> : filteredNotifications.map(notification => <Card key={notification.id} className={`cursor-pointer transition-all hover:bg-accent/50 bg-card border-border ${notification.read ? 'opacity-70' : ''}`} onClick={() => handleNotificationClick(notification)}>
                          <CardContent className="p-4">
                            <div className="flex items-start gap-3">
                              <div className="flex items-center gap-2 mt-1">
                                {notification.priority === 'urgent' ? <AlertTriangle className="h-4 w-4 text-destructive" /> : notification.type === 'success' ? <CheckCircle className="h-4 w-4 text-success" /> : notification.type === 'warning' ? <AlertTriangle className="h-4 w-4 text-warning" /> : notification.type === 'error' ? <XCircle className="h-4 w-4 text-destructive" /> : <Info className="h-4 w-4 text-primary" />}
                              </div>
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center justify-between mt-3">
                                  ...
                                  <div className="flex items-center gap-1">
                                    {notification.actionUrl && <Button variant="ghost" size="sm" onClick={e => {
                                e.stopPropagation();
                                onNavigateToSection?.(notification.actionUrl!);
