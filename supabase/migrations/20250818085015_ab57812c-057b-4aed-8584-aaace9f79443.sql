...
USING (
  auth.uid() = user_id AND (
    ...
    has_role(auth.uid(), 'ap_clerk'::app_role) OR
...
-- Only authorized staff can delete suppliers
...
-- Add procurement_manager to app_role enum if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'app_role') THEN
    CREATE TYPE public.app_role AS ENUM ('admin', 'cfo', 'finance_manager', 'ap_clerk', 'ar_clerk', 'procurement_manager');
  ELSE
    -- Check if procurement_manager value exists
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'procurement_manager' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'app_role')) THEN
