-- إصلاح نظام الجزاءات والمخالفات وفقاً لنظام العمل السعودي (تصحيح الأدوار)
...
INSERT INTO public.saudi_labor_violations (violation_code, category, violation_name, article_reference, description, severity, first_action, second_action, final_action, auto_trigger_rules) VALUES
('ATT001', 'الحضور والانصراف', 'التأخير عن العمل', 'المادة 80', 'التأخير عن موعد بداية العمل المحدد دون عذر مقبول', 'medium', 'إنذار شفهي', 'إنذار كتابي', 'خصم يوم من الراتب', '{"min_occurrences": 3, "period_days": 30, "auto_trigger": true}'),
('ATT002', 'الحضور والانصراف', 'الغياب بدون إذن', 'المادة 80', 'الغياب عن العمل دون إذن مسبق من الرئيس المباشر', 'high', 'إنذار كتابي', 'خصم راتب يوم واحد', 'خصم راتب ثلاثة أيام أو إنهاء الخدمة', '{"min_occurrences": 1, "period_days": 30, "auto_trigger": true}'),
('ATT003', 'الحضور والانصراف', 'الانصراف المبكر بدون إذن', 'المادة 80', 'مغادرة العمل قبل انتهاء الدوام الرسمي دون إذن', 'medium', 'إنذار شفهي', 'إنذار كتابي', 'خصم نصف يوم من الراتب', '{"min_occurrences": 5, "period_days": 30, "auto_trigger": true}'),
('BEH001', 'السلوك المهني', 'عدم احترام الرؤساء أو الزملاء', 'المادة 80', 'التعامل بعدم احترام مع الرؤساء أو الزملاء في العمل', 'high', 'إنذار كتابي', 'خصم راتب يومين', 'إنهاء الخدمة', '{"min_occurrences": 1, "period_days": 365, "auto_trigger": false}'),
('BEH002', 'السلوك المهني', 'استخدام لغة غير لائقة', 'المادة 80', 'استخدام ألفاظ نابية أو غير لائقة في مكان العمل', 'medium', 'إنذار كتابي', 'خصم راتب يوم واحد', 'إنذار نهائي', '{"min_occurrences": 2, "period_days": 180, "auto_trigger": false}'),
('SAF001', 'الأمن والسلامة', 'عدم اتباع إجراءات السلامة', 'المادة 80', 'عدم الالتزام بإجراءات الأمن والسلامة المهنية', 'high', 'إنذار كتابي', 'خصم راتب يوم واحد', 'إنهاء الخدمة', '{"min_occurrences": 1, "period_days": 365, "auto_trigger": false}'),
('PERF001', 'الأداء الوظيفي', 'إهمال في أداء الواجبات', 'المادة 80', 'عدم أداء الواجبات الوظيفية بالجودة المطلوبة', 'medium', 'إنذار شفهي', 'إنذار كتابي', 'خصم راتب أو تدريب إضافي', '{"min_occurrences": 3, "period_days": 90, "auto_trigger": false}'),
('PERF002', 'الأداء الوظيفي', 'عدم تحقيق الأهداف المطلوبة', 'المادة 80', 'فشل متكرر في تحقيق الأهداف والمؤشرات المحددة', 'medium', 'تدريب إضافي', 'إنذار كتابي', 'خصم راتب أو تغيير المنصب', '{"min_occurrences": 2, "period_days": 90, "auto_trigger": false}'),
('POL001', 'سياسة الشركة', 'مخالفة سياسات الشركة', 'المادة 80', 'عدم الالتزام بسياسات وإجراءات الشركة المعتمدة', 'medium', 'إنذار كتابي', 'خصم راتب يوم واحد', 'إنذار نهائي', '{"min_occurrences": 2, "period_days": 60, "auto_trigger": false}'),
('POL002', 'سياسة الشركة', 'إساءة استخدام ممتلكات الشركة', 'المادة 80', 'استخدام معدات أو موارد الشركة لأغراض شخصية', 'high', 'إنذار كتابي', 'خصم راتب يومين', 'إنهاء الخدمة أو استرداد القيمة', '{"min_occurrences": 1, "period_days": 365, "auto_trigger": false}')
...
-- سياسات الأمان للإجراءات التأديبية (استخدام الأدوار الموجودة فقط)
...
-- دالة للتحقق من المخالفات المتكررة في الحضور وإنشاء إجراءات تلقائية
CREATE OR REPLACE FUNCTION check_and_create_attendance_violations(p_employee_id UUID, p_company_id UUID)
RETURNS TABLE (
  violation_type TEXT,
  count INTEGER,
