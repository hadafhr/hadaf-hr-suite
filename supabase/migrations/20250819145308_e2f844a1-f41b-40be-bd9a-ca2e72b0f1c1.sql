-- Create comprehensive leaves and holidays system
...
-- Insert Saudi Labor Law leave types
INSERT INTO saudi_leave_types (code, name_ar, name_en, description_ar, description_en, max_days_per_year, is_paid, requires_medical_certificate, gender_restriction, minimum_service_years, max_consecutive_days) VALUES
...
INSERT INTO saudi_holidays (name_ar, name_en, hijri_date, gregorian_date, duration_days, holiday_type, is_recurring) VALUES
...
-- Company-specific leave policies (customizable)
CREATE TABLE IF NOT EXISTS company_leave_policies (
  ...
  probation_months INTEGER DEFAULT 0, -- Months before eligible
  requires_approval BOOLEAN DEFAULT true,
  approval_levels INTEGER DEFAULT 1,
  blackout_periods JSONB DEFAULT '[]', -- Periods when leave cannot be taken
  gender_restriction TEXT CHECK (gender_restriction IN ('male', 'female', 'both')) DEFAULT 'both',
  job_level_restriction TEXT[],
...
CREATE INDEX idx_leave_requests_company_dates ON leave_requests(company_id, start_date, end_date);
CREATE INDEX idx_leave_balances_employee_year ON employee_leave_balances(employee_id, year);
CREATE INDEX idx_leave_calendar_company_date ON leave_calendar(company_id, event_date);
CREATE INDEX idx_leave_ai_insights_company_type ON leave_ai_insights(company_id, insight_type, is_dismissed);
...
CREATE POLICY "HR can manage company leave policies" ON company_leave_policies
...
CREATE POLICY "Employees can view company leave policies" ON company_leave_policies
...
CREATE POLICY "HR can manage all leave balances" ON employee_leave_balances
  FOR ALL TO authenticated
  USING (company_id = boud_get_user_company_id(auth.uid()) AND 
         (boud_has_role(auth.uid(), company_id, 'super_admin'::user_role) OR 
          boud_has_role(auth.uid(), company_id, 'hr_manager'::user_role)))
  WITH CHECK (company_id = boud_get_user_company_id(auth.uid()) AND 
              (boud_has_role(auth.uid(), company_id, 'super_admin'::user_role) OR 
               boud_has_role(auth.uid(), company_id, 'hr_manager'::user_role)));
...
CREATE POLICY "HR can manage all company leave requests" ON leave_requests
  FOR ALL TO authenticated
  USING (company_id = boud_get_user_company_id(auth.uid()) AND 
         (boud_has_role(auth.uid(), company_id, 'super_admin'::user_role) OR 
          boud_has_role(auth.uid(), company_id, 'hr_manager'::user_role)))
...
CREATE POLICY "Users can view approval history for accessible requests" ON leave_approval_history
  FOR SELECT TO authenticated
  USING (leave_request_id IN (SELECT id FROM leave_requests WHERE 
...
-- Leave calendar
CREATE POLICY "Users can view company leave calendar" ON leave_calendar
  FOR SELECT TO authenticated
  USING (company_id = boud_get_user_company_id(auth.uid()));

CREATE POLICY "HR can manage leave calendar" ON leave_calendar
  FOR ALL TO authenticated
  USING (company_id = boud_get_user_company_id(auth.uid()) AND 
         (boud_has_role(auth.uid(), company_id, 'super_admin'::user_role) OR 
          boud_has_role(auth.uid(), company_id, 'hr_manager'::user_role)))
...
BEGIN
  ...
  WHILE current_date <= end_date LOOP
    ...
    IF day_of_week NOT IN (5, 6) THEN -- Not Friday or Saturday
      ...
      SELECT EXISTS(
        SELECT 1 FROM leave_calendar 
        WHERE company_id = calculate_working_days.company_id 
