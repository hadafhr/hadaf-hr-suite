...
-- Create audit trigger for sensitive data access logging
CREATE OR REPLACE FUNCTION log_hr_employee_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log when someone accesses HR employee data
...
-- Add trigger for SELECT operations (will log when data is accessed)
CREATE TRIGGER hr_employee_access_audit
  AFTER SELECT ON hr_employees
  FOR EACH ROW
