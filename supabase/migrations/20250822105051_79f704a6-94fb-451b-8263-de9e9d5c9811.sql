-- Security Fix: Strengthen boud_payroll_items access controls and add audit logging
...
-- Create audit logging trigger for payroll access
CREATE OR REPLACE FUNCTION log_payroll_data_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log when someone accesses payroll data
...
-- Add triggers for comprehensive payroll access logging
CREATE TRIGGER log_payroll_select_access
  AFTER SELECT ON boud_payroll_items
...
SELECT 
  ...
  pi.overtime_hours,
  pi.created_at,
  -- Only show basic salary and net salary in summary
  CASE 
...
-- Enable RLS on the payroll summary view
ALTER VIEW payroll_summary OWNER TO postgres;

-- Add comment explaining the security measures
COMMENT ON TABLE boud_payroll_items IS 'Secure payroll items table with strict RLS policies: employees can only view their own data, payroll officers and super admins can manage company payroll data, all access is logged for audit purposes.';

COMMENT ON VIEW payroll_summary IS 'Secure payroll summary view that hides sensitive salary details from unauthorized users while allowing basic payroll tracking.';
